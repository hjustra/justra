(function justraPjeCaptureContent() {
  if (window.__justraPjeOperatorCaptureLoaded) {
    window.dispatchEvent(new CustomEvent("justra:pje:operator:open"));
    return;
  }
  window.__justraPjeOperatorCaptureLoaded = true;
  ...
  const OPERATOR_SOURCE = "pje-trt2-operator-chrome-extension";
  const OPERATOR_AUTO_CHECK_MS = 2000;
  const OPERATOR_READY_MIN_TEXT = 700;
  const OPERATOR_READY_MIN_MOVEMENTS = 1;
  ...
  function ensureRoot() {
    ...
    panel.innerHTML = `
      <div class="justra-pje-header">
        <div>
          <h2 class="justra-pje-title">Operador PJe</h2>
          <p class="justra-pje-subtitle">Resolva o CAPTCHA manualmente; a página liberada será enviada à Justra.</p>
        </div>
        <button class="justra-pje-close" type="button" title="Fechar">×</button>
      </div>
      <div class="justra-pje-body">
        <div class="justra-pje-status" data-role="status">Autoenvio ativo quando o processo estiver liberado.</div>
        <div class="justra-pje-actions">
          ...
          <button class="justra-pje-action" type="button" data-action="sendAllDocuments">Enviar agora</button>
        </div>
  ...
  async function getOperatorSettings() {
    const response = await runtimeMessage({ type: "JUSTRA_OPERATOR_GET_SETTINGS" });
    operatorSettings = response.settings || {};
    return operatorSettings;
  }

  function isOperatorEnabled(settings) {
    return Boolean(settings && settings.enabled && settings.autoCapture);
  }

  function payloadFingerprint(payload) {
    const processDigits = payload.process.number_digits || onlyDigits(payload.process.number);
    const candidates = payload.diagnostics.document_candidate_count || 0;
    const movements = payload.movements.length || 0;
    return [
      processDigits || "sem-processo",
      location.pathname,
      candidates,
      movements
    ].join("|");
  ...
  async function recentlySent(fingerprint, settings) {
    const cooldownMs = Math.max(1, Number(settings.resendCooldownMinutes || 60)) * 60 * 1000;
    const stored = await chrome.storage.local.get(["operatorSentFingerprints"]);
    const sent = stored.operatorSentFingerprints || {};
    const row = sent[fingerprint];
    return Boolean(row && Date.now() - Number(row.sent_at || 0) < cooldownMs);
  }

  async function markSent(fingerprint, importResult) {
    const stored = await chrome.storage.local.get(["operatorSentFingerprints"]);
    const sent = stored.operatorSentFingerprints || {};
    sent[fingerprint] = {
      sent_at: Date.now(),
      import_id: importResult.import_id || "",
      process_number: importResult.process_number || ""
    };
    const entries = Object.entries(sent)
      .sort((left, right) => Number(right[1].sent_at || 0) - Number(left[1].sent_at || 0))
      .slice(0, 120);
    await chrome.storage.local.set({ operatorSentFingerprints: Object.fromEntries(entries) });
  }

  function annotateOperatorPayload(payload, settings, trigger) {
    const expectedProcessNumber = onlyDigits(settings.expectedProcessNumber || "");
    payload.capture_mode = "operator_auto";
    payload.job_id = String(settings.jobId || "").trim();
    payload.operator_capture = {
      auto: true,
      trigger,
      expected_process_number: expectedProcessNumber,
      extension_name: chrome.runtime.getManifest().name,
      extension_version: chrome.runtime.getManifest().version,
      captured_page_url: location.href,
      captured_at: new Date().toISOString()
    };
    return payload;
  }

  function pageLooksReadyForOperator(settings) {
    const visibleText = getVisibleText();
    if (hasCaptcha(visibleText)) {
      return { ok: false, reason: "Aguardando CAPTCHA/login manual." };
    }
    if (!/\/consultaprocessual\/(?:captcha\/)?detalhe-processo\//.test(location.pathname)) {
      return { ok: false, reason: "Aguardando página de detalhe do processo." };
    }
    const processNumber = extractProcessNumber([location.href, document.title, visibleText].join("\n"));
    const processDigits = onlyDigits(processNumber);
    if (!processDigits || processDigits.length !== 20) {
      return { ok: false, reason: "Aguardando CNJ visível na página." };
    }
    const expected = onlyDigits(settings.expectedProcessNumber || "");
    if (expected && expected !== processDigits) {
      return { ok: false, reason: `CNJ aberto não confere com o esperado: ${formatCnj(processDigits)}.` };
    }
    if (visibleText.length < OPERATOR_READY_MIN_TEXT) {
      return { ok: false, reason: "Aguardando conteúdo do PJe carregar." };
    }
    const movements = collectMovements(visibleText);
    if (movements.length < OPERATOR_READY_MIN_MOVEMENTS) {
      return { ok: false, reason: "Aguardando movimentos do processo." };
    }
    return { ok: true, processNumber, processDigits };
  }

  function setOperatorLauncher(message, kind = "info") {
    ensureRoot();
    launcher.textContent = kind === "success" ? "Justra OK" : kind === "warning" ? "Justra espera" : kind === "error" ? "Justra erro" : "Justra Auto";
    launcher.title = message || "Coleta automática de operador Justra";
  }

  async function operatorAutoCapture(trigger = "auto") {
    if (autoCaptureRunning) {
      return;
    }
    const settings = await getOperatorSettings();
    if (!isOperatorEnabled(settings)) {
      setOperatorLauncher("Autoenvio de operador desativado.", "info");
      return;
    }
    const ready = pageLooksReadyForOperator(settings);
    if (!ready.ok) {
      setOperatorLauncher(ready.reason, ready.reason.includes("CAPTCHA") ? "warning" : "info");
      return;
    }
    autoCaptureRunning = true;
    try {
      const settleMs = Math.max(500, Number(settings.settleDelayMs || 2500));
      setOperatorLauncher(`Página liberada. Capturando em ${Math.round(settleMs / 1000)}s...`, "info");
      await sleep(settleMs);
      const readyAfterSettle = pageLooksReadyForOperator(settings);
      if (!readyAfterSettle.ok) {
        setOperatorLauncher(readyAfterSettle.reason, "warning");
        return;
      }
      setStatus("Operador: capturando e enviando automaticamente...", "info");
      const payload = annotateOperatorPayload(
        await buildPayloadWithAllVisibleDocuments((progress) => {
          setOperatorLauncher(`Capturando documento ${progress.index}/${progress.total}`, "info");
        }),
        settings,
        trigger
      );
      const fingerprint = payloadFingerprint(payload);
      if (fingerprint === lastAutoFingerprint || await recentlySent(fingerprint, settings)) {
        setOperatorLauncher("Este processo já foi enviado recentemente.", "success");
        return;
      }
      const importResult = await sendPayloadToJustra(payload);
      lastAutoFingerprint = fingerprint;
      await markSent(fingerprint, importResult);
      setOperatorLauncher(`Enviado para ${importResult.import_environment_label || "Justra"}.`, "success");
    } catch (error) {
      setOperatorLauncher(`Falha no autoenvio: ${error.message}`, "error");
      setStatus(`Operador: falha no autoenvio: ${error.message}`, "error");
    } finally {
      autoCaptureRunning = false;
    }
  }

  function startOperatorAutoCaptureLoop() {
    window.setTimeout(() => operatorAutoCapture("initial-load").catch(() => {}), 800);
    window.setInterval(() => operatorAutoCapture("poll").catch(() => {}), OPERATOR_AUTO_CHECK_MS);
  }
  ...
  function importPayloadIntoJustra(payload) {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({ type: "JUSTRA_OPERATOR_IMPORT_PJE", payload }, (response) => {
        if (chrome.runtime.lastError) {
          reject(new Error(chrome.runtime.lastError.message));
          return;
        }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message.type === "JUSTRA_OPERATOR_CAPTURE_NOW") {
      openPanel();
      getOperatorSettings()
        .then((settings) => buildPayloadWithAllVisibleDocuments((progress) => {
          setStatus(`Operador: abrindo documento ${progress.index}/${progress.total}: ${progress.candidate.title || progress.candidate.code}`, "info");
        }).then((payload) => annotateOperatorPayload(payload, settings, "popup")))
        .then((payload) => sendPayloadToJustra(payload).then((importResult) => ({
          payload,
          importResult
        })))
        .then(({ payload, importResult }) => sendResponse({
  ...
  window.addEventListener("justra:pje:operator:open", openPanel);
  ensureRoot();
  startOperatorAutoCaptureLoop();
})();
